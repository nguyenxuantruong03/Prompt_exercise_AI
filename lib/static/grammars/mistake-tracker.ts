...
export class MistakeTracker {
  ...
  private getTopicMasteryChart(mistakes: MistakeRecord[]): ChartData {
    ...
    Object.entries(topicGroups).forEach(([topic]) => {
  ...
  getMiniExerciseSuggestion(grammarTopic: string): {
    ...
    const suggestion = this.generateMiniExerciseSuggestion(grammarTopic);
  ...
  private generateMiniExerciseSuggestion(grammarTopic: string): {
    title: string;
    description: string;
    exercises: Array<{
      type: string;
  ...
  getPracticeRecommendations(): PracticeRecommendations {
    ...
    const urgentTopics = Object.entries(topicMistakes)
      .map(([topic, topicMistakeList]) => {
        ...
        const suggestion = this.generateMiniExerciseSuggestion(topic);
